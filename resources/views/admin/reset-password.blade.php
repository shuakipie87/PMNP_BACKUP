...
@section('content')
    ...
    <form method="POST" action="{{ route('password.update', ['page' => true]) }}" class="register needs-validation" autocomplete="off">
        ...
        @if($errors->has('email'))
            <div class="mt-1 mb-2 alert alert-danger">{{ $errors->first('email') }}</div>
        @endif
        ...
        <div class="text-center pt-1 mt-5 pb-1">
            <input type="submit" value="Reset Password" class="btn btn-block fa-lg gradient-custom-2 mb-3 text-white font-weight-bold" />
        </div>
